...
export function RequestOtpScreen() {
    ...
    try {
      ...
      setServerError(
        error instanceof Error
          ? error.message
          : "Could not send the code. Contact an administrator if this continues.",
      );
    }
  }
  ...
  return (
    <View className="flex-1 justify-center bg-neutral-50 px-6">
      <Text className="text-3xl font-bold text-primary-700">Technician Login</Text>
      ...
      <View className="mt-8">
        ...
        />
        {errors.phone ? <Text className="mt-1 text-sm text-error">{errors.phone.message}</Text> : null}
      </View>
