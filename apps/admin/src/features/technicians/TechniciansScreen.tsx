...
export function TechniciansScreen() {
  ...
  const query = useTechniciansQuery({
    page,
    pageSize: PAGE_SIZE,
  ...
  async function onCreateSubmit(values: CreateAdminUserFormValues): Promise<void> {
    try {
      ...
      setCreateOpen(false);
      show("Technician added.", "success");
    } catch (error) {
      show(errorMessage(error, "Could not add technician."), "error");
    }
  }
  ...
  async function onEditSubmit(values: UpdateAdminUserFormValues): Promise<void> {
    ...
    try {
      ...
      setEditingUser(null);
      show("Technician updated.", "success");
    } catch (error) {
      show(errorMessage(error, "Could not update technician."), "error");
    }
  }
  ...
  async function confirmStatusChange(): Promise<void> {
    ...
    try {
      ...
      show(statusTarget.isActive ? "Technician deactivated." : "Technician reactivated.", "success");
    } catch (error) {
  ...
  return (
    <View className="flex-1 bg-neutral-50 p-6">
      <View className="mb-4 flex-row items-center justify-between">
        <Text className="text-xl font-semibold text-neutral-900">Technicians</Text>
      ...
      <Table
        ...
        error={query.isError ? "Could not load technicians." : undefined}
        onRetry={() => query.refetch()}
        emptyTitle="No technicians yet"
        emptyDescription="Technicians you add will appear here."
        sortKey={sortBy}
      ...
      <Dialog
        ...
        title={statusTarget?.isActive ? "Deactivate technician?" : "Reactivate technician?"}
        description={
          statusTarget?.isActive
            ? `${statusTarget.name ?? "This technician"} will no longer be able to log in.`
            : `${statusTarget?.name ?? "This technician"} will be able to log in again.`
        }
